...
-- 計算欄位資訊使用
create temporary table BB as select page,row,col,user_id,max(txn_no) as max_txn from govcash_txn group by page,row,col,user_id;
create temporary table BB2 as select  A.* from govcash_txn as A inner join BB as B on (A.page =B.page and A.row = B.row and A.col=B.col and  A.user_id = B.user_id and A.txn_no = B.max_txn);

create temporary table B3 as select  A.page,A.row,A.col,A.user_id,case when B.ans is not null then B.ans else A.ans end as ans,a.original_ans,case when B.txn_no is not null then b.txn_no else A.txn_no end as txn_no,case when B.time is not null then B.time else A.time end as time from BB2 as A left join B2 as B on (A.page =B.page and A.row = B.row and A.col=B.col and  A.user_id = B.user_id);
